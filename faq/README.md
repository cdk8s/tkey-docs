...
会 Spring Boot 的人自己价格 Mapper 之后去查数据库，这个真的不难吧。
...
大家如果看过 TKey 的压力测试文档内容，会知道目前主要的瓶颈开销在 Redis 上。

但是，我是坚定要上存储介质的，可以不是 Redis，可以换成：LevelDB、SSDB、RocksDB、Memcached 等等。
可是必然要有一个存储介质来存储 Token 信息。不然无法解决任意横向扩展服务这件事。

因为引入了 Redis，所以 TKey 的一个核心点就是维护好：Redis 的高可用，保持好网络通畅。

小公司自己维护 Redis 没什么问题，因为量不大，如果已经成规模，建议直接采用云上 Redis，尽可能保住高可用。
...
如果我部分文档或测试采用 URL 方式，那只有一个原因：为了方便。大家不要学习。
...
如果没有 API 网关，则这里就要引入 MQ。每个业务系统监听 TKey 的 MQ 通道，当发现各自的业务系统中有当前退出用户的 Token 则要自己进行销毁。但是这样的设计复杂度就上来了，不划算。所以，这里又一次提到了 API 网关。
...
第二种情况 Cookie、Session 是完全用不了的，只能把 Token 信息交给前端存储在 LocalStorage 中，请求的时候带上请求头。推荐大家使用这种。
